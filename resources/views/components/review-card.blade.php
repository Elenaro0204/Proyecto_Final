    ...
    <div class="p-4 flex flex-col flex-1">
        ...
        <div class="flex gap-2 mt-auto items-stretch">
            ...
            <button @click="openModal({{ $review->id }})"
                class="flex-1 bg-indigo-600 text-white hover:bg-indigo-700 px-3 py-2 rounded text-center transition text-sm">
                Ver
            </button>
            ...
            @auth
                ...
                @if (Auth::id() === $review->user_id)
                    ...
                    @if (!Auth::user()->isAdmin())
                        <form action="{{ route('resenas.destroy', $review->id) }}" method="POST" class="flex-1"
                ...
                @if (Auth::user()->isAdmin())
                    ...
                    @else
                        <form action="{{ route('admin.resenas.report.cancel', $userReport->id) }}" method="POST"
                            ...
                            @method('DELETE')
                            <button type="submit"
                                class="flex-1 bg-gray-400 text-black hover:bg-gray-500 px-3 py-2 rounded text-center transition text-sm">
                                Cancelar reporte
                            </button>
                        </form>
                    @endif
