<!-- resources/views/admin/foros/viewreport.blade.php -->
    ...
    <div class="container mx-auto py-10 px-4">
        <div class="max-w-3xl mx-auto relative rounded-xl overflow-hidden shadow-xl p-6">
            ...
            <div class="relative z-10 px-8 py-6">
                ...
                @if ($report)
                    <div class="relative z-10 bg-white shadow-lg rounded-xl p-6 mb-6 border border-gray-200">
                        <div class="flex flex-col md:flex-row md:justify-between md:items-start gap-4">

                            {{-- Información del foro --}}
                            <div class="flex-1">
                                <h2 class="text-xl font-bold text-gray-800 mb-2">
                                    {{ $report->foro->title ?? "Foro #{$report->foro->id}" }}
                                </h2>
                            ...
                            <div class="flex flex-col items-center md:items-end gap-2">
                                ...
                                {{-- Acciones --}}
                                <div class="flex flex-col justify-between mt-4 gap-3">
                                    <a href="{{ url()->previous() }}"
                                        class="px-4 py-2 rounded bg-gray-200 hover:bg-gray-300 text-center">
                                        Cancelar
                                    </a>

                                    <form action="{{ route('admin.foro.report.cancel', $report->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                ...
                @else
                    <p class="text-gray-600">Este foro no tiene reportes.</p>
                @endif
            </div>
        </div>
    </div>
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            function updateCountdown(el) {
                ...
                if (years > 0) parts.push(
                    `<span class="bg-yellow-200 text-yellow-800 px-2 py-1 rounded-lg">${years}a</span>`);
                if (months > 0) parts.push(
                    `<span class="bg-blue-200 text-blue-800 px-2 py-1 rounded-lg">${months}m</span>`);
                if (days > 0) parts.push(
                    `<span class="bg-green-200 text-green-800 px-2 py-1 rounded-lg">${days}d</span>`);
