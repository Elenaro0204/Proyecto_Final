    ...
    <script>
        ...
        <div class="border rounded-lg p-3 bg-white shadow hover:shadow-lg transition flex flex-col h-full">
            ...
            function renderSection(gridEl, items, type) {
                ...
                items.forEach(item => {
                    ...
                    if (type === 'personajes') {
                        ...
                        subtitle = item.description || '';
                        link = `/comic/${item.id}`;
                    } else if (type === 'series' || type === 'peliculas') {
                        img = item.Poster && item.Poster !== 'N/A' ? item.Poster :
                            '/images/default-movie.png';
                        title = item.Title || 'Sin título';
                        subtitle = item.Year ? `Año: ${item.Year}` : '';
                        link = type === 'series' ? `/serie/${item.imdbID}` : `/pelicula/${item.imdbID}`;
                    }
            ...
            async function doSearch(q) {
                ...
                try {
                    const url = new URL("{{ route('buscar.ajax') }}", window.location.origin);
                    ...
                    if ((json.personajes?.length || 0) + (json.comics?.length || 0) + (json.series?.length ||
                            0) + (json.peliculas?.length || 0) === 0) {
