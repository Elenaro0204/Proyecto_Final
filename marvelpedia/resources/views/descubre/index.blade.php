...
@section('content')
<x-welcome-section
    title="Explora el Multiverso Marvel"
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            ...
            @foreach($personajesCollection->take(8) as $personaje)
                <a href="{{ route('personaje.show', $personaje['id']) }}">
                    <div class="relative border rounded-xl overflow-hidden shadow-lg transform hover:scale-105 transition duration-300 bg-white">
                ...
                <a href="{{ route('serie.show', $serie['id']) }}">
                    <div class="relative border rounded-xl overflow-hidden shadow-lg transform hover:scale-105 transition duration-300 bg-white">
                        <img class="w-full h-52 object-cover"
                             ...
                             alt="{{ $serie['title'] }}">
                        <div class="absolute bottom-0 w-full bg-gradient-to-t from-black/70 to-transparent p-3">
                            <h2 class="text-white font-semibold text-lg truncate">{{ $serie['title'] }}</h2>
                        </div>
                    </div>
                </a>
            @endforeach
