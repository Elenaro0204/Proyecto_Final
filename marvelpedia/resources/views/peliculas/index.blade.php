...
@section('content')
<x-welcome-section
    title="Películas del Multiverso Marvel"
        ...
        @foreach($peliculas as $pelicula)
            <div class="border rounded-lg p-4 bg-white shadow hover:shadow-lg transition flex flex-col justify-between h-full">
                ...
                <div class="flex-1 mt-2">
                   ...
                   @if(!empty($pelicula['description_es']))
                        <p class="text-sm text-gray-600 line-clamp-3">{{ $pelicula['description_es'] }}</p>
                    @else
                ...
                if (data.length) {
                    data.forEach(p => {
                        ...
                        div.addEventListener('click', () => {
                            window.location.href = `/pelicula/${p.id}`;
                        });
